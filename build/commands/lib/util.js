...
async function applyPatches() {
  ...
  const coreRepoPath = config.adrbrowsielCoreDir
...
const util = {
  ...
  buildGClientConfig: () => {
    ...
    const solutions = [
      ...
      {
        managed: "%False%",
        name: "src/adrbrowsiel",
        url: config.adrbrowsielCoreRepo
      }
    ]
  ...
  updateBranding: () => {
    ...
    const adrbrowsielComponentsDir = path.join(config.adrbrowsielCoreDir, 'components')
    const chromeAppDir = path.join(config.srcDir, 'chrome', 'app')
    const adrbrowsielAppDir = path.join(config.adrbrowsielCoreDir, 'app')
    const chromeBrowserResourcesDir = path.join(config.srcDir, 'chrome', 'browser', 'resources')
    const adrbrowsielBrowserResourcesDir = path.join(config.adrbrowsielCoreDir, 'browser', 'resources')
    const adrbrowsielAppVectorIconsDir = path.join(config.adrbrowsielCoreDir, 'vector_icons', 'chrome', 'app')
    const chromeAndroidJavaStringsTranslationsDir = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'strings', 'translations')
    const adrbrowsielAndroidJavaStringsTranslationsDir = path.join(config.adrbrowsielCoreDir, 'browser', 'ui', 'android', 'strings', 'translations')

    let fileMap = new Set();
    const autoGeneratedadrbrowsielToChromiumMapping = Object.assign({}, l10nUtil.getAutoGeneratedadrbrowsielToChromiumMapping())
    // The following 3 entries we map to the same name, not the chromium equivalent name for copying back
    autoGeneratedadrbrowsielToChromiumMapping[path.join(adrbrowsielAppDir, 'adrbrowsiel_strings.grd')] = path.join(chromeAppDir, 'adrbrowsiel_strings.grd')
    autoGeneratedadrbrowsielToChromiumMapping[path.join(adrbrowsielAppDir, 'settings_adrbrowsiel_strings.grdp')] = path.join(chromeAppDir, 'settings_adrbrowsiel_strings.grdp')
    autoGeneratedadrbrowsielToChromiumMapping[path.join(adrbrowsielComponentsDir, 'components_adrbrowsiel_strings.grd')] = path.join(chromeComponentsDir, 'components_adrbrowsiel_strings.grd')

    Object.entries(autoGeneratedadrbrowsielToChromiumMapping).forEach(mapping => fileMap.add(mapping))

    // Copy xtb files for:
    // adrbrowsiel/app/resources/chromium_strings*.xtb
    // adrbrowsiel/app/resources/generated_resoruces*.xtb
    // adrbrowsiel/components/strings/components_chromium_strings*.xtb
    // adrbrowsiel/browser/ui/android/strings/translations/android_chrome_strings*.xtb
    fileMap.add([path.join(adrbrowsielAppDir, 'resources'), path.join(chromeAppDir, 'resources')])
    fileMap.add([path.join(adrbrowsielComponentsDir, 'strings'), path.join(chromeComponentsDir, 'strings')])
    fileMap.add([adrbrowsielAndroidJavaStringsTranslationsDir, chromeAndroidJavaStringsTranslationsDir])
    // By overwriting, we don't need to modify some grd files.
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'adrbrowsiel'), path.join(chromeAppDir, 'theme', 'adrbrowsiel')])
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'adrbrowsiel'), path.join(chromeAppDir, 'theme', 'chromium')])
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'default_100_percent', 'adrbrowsiel'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'adrbrowsiel')])
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'default_200_percent', 'adrbrowsiel'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'adrbrowsiel')])
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'default_100_percent', 'adrbrowsiel'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'default_200_percent', 'adrbrowsiel'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'default_100_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_100_percent', 'common')])
    fileMap.add([path.join(adrbrowsielAppDir, 'theme', 'default_200_percent', 'common'), path.join(chromeAppDir, 'theme', 'default_200_percent', 'common')])
    fileMap.add([path.join(adrbrowsielComponentsDir, 'resources', 'default_100_percent'), path.join(chromeComponentsDir, 'resources', 'default_100_percent')])
    fileMap.add([path.join(adrbrowsielComponentsDir, 'resources', 'default_100_percent', 'adrbrowsiel'), path.join(chromeComponentsDir, 'resources', 'default_100_percent', 'chromium')])
    fileMap.add([path.join(adrbrowsielComponentsDir, 'resources', 'default_200_percent'), path.join(chromeComponentsDir, 'resources', 'default_200_percent')])
    fileMap.add([path.join(adrbrowsielComponentsDir, 'resources', 'default_200_percent', 'adrbrowsiel'), path.join(chromeComponentsDir, 'resources', 'default_200_percent', 'chromium')])
    fileMap.add([path.join(adrbrowsielAppVectorIconsDir, 'vector_icons', 'adrbrowsiel'), path.join(chromeAppDir, 'vector_icons', 'adrbrowsiel')])
    // Copy chrome-logo-faded.png for replacing chrome logo of welcome page with adrbrowsiel's on Win8.
    fileMap.add([path.join(adrbrowsielBrowserResourcesDir, 'chrome-logo-faded.png'), path.join(chromeBrowserResourcesDir, 'chrome-logo-faded.png')])
    fileMap.add([path.join(adrbrowsielBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg'), path.join(chromeBrowserResourcesDir, 'downloads', 'images', 'incognito_marker.svg')])
    fileMap.add([path.join(adrbrowsielBrowserResourcesDir, 'settings', 'images'), path.join(chromeBrowserResourcesDir, 'settings', 'images')])
    fileMap.add([path.join(adrbrowsielBrowserResourcesDir, 'signin', 'profile_picker', 'images'), path.join(chromeBrowserResourcesDir, 'signin', 'profile_picker', 'images')])
    // Copy to make our ${branding_path_component}_behaviors.cc
    fileMap.add([path.join(config.adrbrowsielCoreDir, 'chromium_src', 'chrome', 'installer', 'setup', 'adrbrowsiel_behaviors.cc'),
                 path.join(config.srcDir, 'chrome', 'installer', 'setup', 'adrbrowsiel_behaviors.cc')])
    // Replace webui CSS to use our fonts.
    fileMap.add([path.join(config.adrbrowsielCoreDir, 'ui', 'webui', 'resources', 'css', 'text_defaults_md.css'),
    ...
    if (process.platform === 'darwin') {
      ...
      // Each channel's app icons are stored in adrbrowsiel/app/theme/$channel/app.icns.
      // With this copying, we don't need to modify chrome/BUILD.gn for this.
      const iconSource = path.join(adrbrowsielAppDir, 'theme', 'adrbrowsiel', 'mac', config.channel, 'app.icns')
      const iconDest = path.join(chromeAppDir, 'theme', 'adrbrowsiel', 'mac', 'app.icns')
      ...
      const brandingSource = path.join(adrbrowsielAppDir, 'theme', 'adrbrowsiel', branding_file_name)
      const brandingDest = path.join(chromeAppDir, 'theme', 'adrbrowsiel', 'BRANDING')
    ...
    if (config.targetOS === 'android') {
      ...
      if (config.channel === 'development') {
        androidIconSet = 'res_adrbrowsiel_default'
      } else if (config.channel === '') {
        androidIconSet = 'res_adrbrowsiel'
      } else if (config.channel === 'beta') {
        androidIconSet = 'res_adrbrowsiel_beta'
      } else if (config.channel === 'dev') {
        androidIconSet = 'res_adrbrowsiel_dev'
      } else if (config.channel === 'nightly') {
        androidIconSet = 'res_adrbrowsiel_nightly'
      }

      const androidIconSource = path.join(adrbrowsielAppDir, 'theme', 'adrbrowsiel', 'android', androidIconSet)
      const androidIconDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_chromium')
      const androidIconBaseSource = path.join(adrbrowsielAppDir, 'theme', 'adrbrowsiel', 'android', androidIconSet + '_base')
      const androidIconBaseDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_chromium_base')
      const androidResSource = path.join(config.adrbrowsielCoreDir, 'android', 'java', 'res')
      const androidResDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res')
      const androidResTemplateSource = path.join(config.adrbrowsielCoreDir, 'android', 'java', 'res_template')
      const androidResTemplateDest = path.join(config.srcDir, 'chrome', 'android', 'java', 'res_template')
      const androidContentPublicResSource = path.join(config.adrbrowsielCoreDir, 'content', 'public', 'android', 'java', 'res')
      const androidContentPublicResDest = path.join(config.srcDir, 'content', 'public', 'android', 'java', 'res')
      const androidTouchtoFillResSource = path.join(config.adrbrowsielCoreDir, 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidTouchtoFillResDest = path.join(config.srcDir, 'chrome', 'browser', 'touch_to_fill', 'android', 'internal', 'java', 'res')
      const androidToolbarResSource = path.join(config.adrbrowsielCoreDir, 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidToolbarResDest = path.join(config.srcDir, 'chrome', 'browser', 'ui', 'android', 'toolbar', 'java', 'res')
      const androidComponentsResSource = path.join(config.adrbrowsielCoreDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')
      const androidComponentsResDest = path.join(config.srcDir, 'components', 'browser_ui', 'widget', 'android', 'java', 'res')
      const androidSafeBrowsingResSource = path.join(config.adrbrowsielCoreDir, 'browser', 'safe_browsing', 'android', 'java', 'res')
      const androidSafeBrowsingResDest = path.join(config.srcDir, 'chrome', 'browser', 'safe_browsing', 'android', 'java', 'res')

      // Mapping for copying adrbrowsiel's Android resource into chromium folder.
  ...
  updateOmahaMidlFiles: () => {
    ...
    const srcDir = path.join(config.adrbrowsielCoreDir, 'win_build_output', 'midl', 'google_update')
  ...
  signWinBinaries: () => {
    ...
    fs.copySync(path.join(config.outputDir, 'adrbrowsiel.exe'), path.join(dir, 'adrbrowsiel.exe'));
    fs.copySync(path.join(config.outputDir, 'chrome.dll'), path.join(dir, 'chrome.dll'));

    util.run('python', [path.join(config.adrbrowsielCoreDir, 'script', 'sign_binaries.py'), '--build_dir=' + dir])
  },
  ...
  generateWidevineSigFiles: () => {
    ...
    util.run('python', [sig_generator, '--input_file=' + path.join(src_dir, 'adrbrowsiel.exe'),
        '--flags=1',
        '--certificate=' + cert,
        '--private_key=' + key,
        '--output_file=' + path.join(config.outputDir, 'adrbrowsiel.exe.sig'),
  ...
  buildRedirectCCTool: () => {
    ...
    const redirectCCExe = path.join(config.adrbrowsielCoreDir, 'buildtools', 'win', 'redirect-cc', 'bin', 'redirect-cc.exe')
    ...
    const msBuildArgs = [
      path.join(config.adrbrowsielCoreDir, 'buildtools', 'win', 'redirect-cc', 'redirect-cc.sln'),
  ...
  generateXcodeWorkspace: (options = config.defaultOptions) => {
    ...
    const args = util.buildArgsToString(config.buildArgs())
    const genScript = path.join(config.adrbrowsielCoreDir, 'vendor', 'gn-project-generators', 'xcode.py')
  ...
  lint: (options = {}) => {
    ...
    cmd_options.cwd = config.adrbrowsielCoreDir
    cmd_options = mergeWithDefault(cmd_options)
    util.run('vpython', [path.join(config.adrbrowsielCoreDir, 'build', 'commands', 'scripts', 'lint.py'),
  ...
  format: (options = {}) => {
    ...
    cmd_options.cwd = config.adrbrowsielCoreDir
    cmd_options = mergeWithDefault(cmd_options)
    util.run('vpython', [path.join(config.adrbrowsielCoreDir, 'build', 'commands', 'scripts', 'format.py'), options.full ? '--full' : ''], cmd_options)
  },
  ...
  gclientSync: (forceReset = false, cleanup = false, adrbrowsielCoreRef = null, options = {}) => {
    ...
    if (forceReset || adrbrowsielCoreRef) {
      if (!adrbrowsielCoreRef) {
        // Use current branch (sync will then pull latest) or current exact hash
        // if we're not in a branch.
        adrbrowsielCoreRef = util.runGit(config.adrbrowsielCoreDir, ['rev-parse', '--abbrev-ref', 'HEAD'])
        if (adrbrowsielCoreRef === 'HEAD') {
          // get the rev hash if we're not in a branch
          adrbrowsielCoreRef = util.runGit(config.adrbrowsielCoreDir, ['rev-parse', 'HEAD'])
        }
      }

      args = args.concat(['--revision', 'src/adrbrowsiel@' + adrbrowsielCoreRef])
      reset = true
    }
    ...
    if (forceReset) {
      ...
      if (cleanup) {
        // temporarily ignored until we can figure out how not to delete src/adrbrowsiel in the process
        // args = args.concat(['-D'])
      }
    }
