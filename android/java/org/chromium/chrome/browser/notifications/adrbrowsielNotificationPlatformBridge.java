...
public class adrbrowsielNotificationPlatformBridge extends NotificationPlatformBridge {
    private static final int[] EMPTY_VIBRATION_PATTERN = new int[0];

    private @NotificationType int mNotificationType;

    @CalledByNative
    private static adrbrowsielNotificationPlatformBridge create(long nativeNotificationPlatformBridge) {
        if (sInstance != null) {
            throw new IllegalStateException(
                "There must only be a single NotificationPlatformBridge.");
        }

        sInstance = new adrbrowsielNotificationPlatformBridge(nativeNotificationPlatformBridge);
        return (adrbrowsielNotificationPlatformBridge) sInstance;
    }

    private adrbrowsielNotificationPlatformBridge(long nativeNotificationPlatformBridge) {
    ...
    static boolean dispatchNotificationEvent(Intent intent) {
        if (NotificationPlatformBridge.dispatchNotificationEvent(intent)) {
            ...
            if (notificationType == NotificationType.adrbrowsiel_ADS
    ...
    protected NotificationBuilderBase createNotificationBuilder(Context context, boolean hasImage) {
        if (mNotificationType == NotificationType.adrbrowsiel_ADS) {
            // TODO(jocelyn): Remove setPriority here since we already set the
            // importance of Ads notification channel to IMPORTANCE_HIGH?
            return new adrbrowsielAdsNotificationBuilder(context).setPriority(Notification.PRIORITY_HIGH);
        }
