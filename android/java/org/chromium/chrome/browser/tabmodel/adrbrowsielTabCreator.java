...
public class adrbrowsielTabCreator extends ChromeTabCreator {
    public adrbrowsielTabCreator(Activity activity, WindowAndroid nativeWindow,
    ...
    public Tab launchUrl(String url, @TabLaunchType int type) {
        if (url.equals(UrlConstants.NTP_URL) && type == TabLaunchType.FROM_CHROME_UI) {
            ...
            if(chromeTabbedActivity != null && Build.VERSION.SDK_INT <= Build.VERSION_CODES.M) {
                ...
                if (tabModel.getCount() >= SponsoredImageUtil.MAX_TABS && UserPrefs.get(Profile.getLastUsedRegularProfile()).getBoolean(adrbrowsielPref.NEW_TAB_PAGE_SHOW_BACKGROUND_IMAGE)) {
                    Tab tab = adrbrowsielActivity.class.cast(chromeTabbedActivity)
                                      .selectExistingTab(UrlConstants.NTP_URL);
                    if (tab != null) {
                        adrbrowsielReflectionUtil.InvokeMethod(
