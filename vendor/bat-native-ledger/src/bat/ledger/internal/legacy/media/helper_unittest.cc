...
// npm run test -- adrbrowsiel_unit_tests --filter=MediaHelperTest.*
...
TEST(MediaHelperTest, GetMediaKey) {
  // provider is missing
  std::string result = adrbrowsielledger_media::GetMediaKey("key", "");
  ASSERT_EQ(result, "");

  // key is missing
  result = adrbrowsielledger_media::GetMediaKey("", "youtube");
  ASSERT_EQ(result, "");

  // all ok
  result = adrbrowsielledger_media::GetMediaKey("key", "youtube");
...
TEST(MediaHelperTest, GetTwitchParts) {
  ...
  // string is empty
  adrbrowsielledger_media::GetTwitchParts("", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);

  // random string
  adrbrowsielledger_media::GetTwitchParts("this is random string", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);
  ...
  // all ok
  adrbrowsielledger_media::GetTwitchParts(post_data, &twitch_parts);
...
//  // string empty
  std::string result = adrbrowsielledger_media::ExtractData("", "/", "!");
  ASSERT_EQ(result, "");

  // missing start
  result = adrbrowsielledger_media::ExtractData("st/find/me!", "", "!");
