...
// npm run test -- adrbrowsiel_unit_tests --filter=BatHelperTest.*
...
TEST(BatHelperTest, HasSameDomainAndPath) {
  ...
  // regular url
  bool result = adrbrowsielledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
  ASSERT_EQ(result, true);

  // empty url with portion
  url = std::string();
  result = adrbrowsielledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
