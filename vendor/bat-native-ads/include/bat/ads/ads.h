...
using GetBraveNewsAdCallback =
    std::function<void(const bool, const std::string&, const BraveNewsAdInfo&)>;
...
class ADS_EXPORT Ads {
 public:
  ...
  // Should be called to get an eligible Brave News ad for the specified size
  virtual void GetBraveNewsAd(const std::string& size,
                              GetBraveNewsAdCallback callback) = 0;

  // Should be called when a user views or clicks a Brave News ad
  virtual void OnBraveNewsAdEvent(const std::string& uuid,
                                  const std::string& creative_instance_id,
                                  const BraveNewsAdEventType event_type) = 0;
