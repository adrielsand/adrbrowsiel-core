...
BraveNewsAdInfo::BraveNewsAdInfo() = default;

BraveNewsAdInfo::BraveNewsAdInfo(const BraveNewsAdInfo& info) = default;

BraveNewsAdInfo::~BraveNewsAdInfo() = default;

bool BraveNewsAdInfo::IsValid() const {
  if (!AdInfo::IsValid()) {
    return false;
  }
...
std::string BraveNewsAdInfo::ToJson() const {
  std::string json;
  SaveToJson(*this, &json);
  return json;
}

Result BraveNewsAdInfo::FromJson(const std::string& json) {
...
void SaveToJson(JsonWriter* writer, const BraveNewsAdInfo& info) {
  writer->StartObject();
