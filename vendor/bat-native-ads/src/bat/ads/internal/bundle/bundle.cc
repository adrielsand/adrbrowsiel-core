...
void Bundle::BuildFromCatalog(const Catalog& catalog) {
  ...
  SaveCreativeBraveNewsAds(bundle_state.creative_brave_news_ads);
...
BundleState Bundle::FromCatalog(const Catalog& catalog) const {
  ...
  for (const auto& campaign : catalog.GetCampaigns()) {
    ...
    for (const auto& creative_set : campaign.creative_sets) {
      ...
      // Brave News ad creatives
      for (const auto& creative : creative_set.creative_brave_news_ads) {
        if (!DoesOsSupportCreativeSet(creative_set)) {
          const std::string platform_name =
              PlatformHelper::GetInstance()->GetPlatformName();
        ...
        for (const auto& segment : creative_set.segments) {
          ...
          info.segment = segment_name;
          creative_brave_news_ads.push_back(info);
          entries++;
...
void Bundle::DeleteCreativeBraveNewsAds() {
  database::table::CreativeBraveNewsAds database_table;
  database_table.Delete([](const Result result) {
    if (result != SUCCESS) {
      BLOG(0, "Failed to delete creative Brave News ads state");
      return;
    }

    BLOG(3, "Successfully deleted creative Brave News ads state");
  });
}
...
void Bundle::SaveCreativeBraveNewsAds(
    const CreativeBraveNewsAdList& creative_brave_news_ads) {
  database::table::CreativeBraveNewsAds database_table;

  database_table.Save(creative_brave_news_ads, [](const Result result) {
    if (result != SUCCESS) {
      BLOG(0, "Failed to save creative Brave News ads state");
      return;
    }

    BLOG(3, "Successfully saved creative Brave News ads state");
  });
}
