...
// #include "bat/ads/internal/eligible_ads/brave_news_ads/seen_ads.h"
// #include "bat/ads/internal/eligible_ads/brave_news_ads/seen_advertisers.h"
...
bool ShouldCapLastServedAd(const CreativeBraveNewsAdList& ads) {
  return ads.size() != 1;
}
...
void EligibleAds::GetForParentChildSegments(
  ...
  database_table.GetForSegments(
      segments, [=](const Result result, const SegmentList& segments,
        ...
        CreativeBraveNewsAdList eligible_ads =
...
CreativeBraveNewsAdList EligibleAds::FilterIneligibleAds(
    const CreativeBraveNewsAdList& ads,
    const AdEventList& ad_events,
    const BrowsingHistoryList& history) const {
  CreativeBraveNewsAdList eligible_ads = ads;
  if (eligible_ads.empty()) {
    return eligible_ads;
  }
...
CreativeBraveNewsAdList EligibleAds::ApplyFrequencyCapping(
    const CreativeBraveNewsAdList& ads,
