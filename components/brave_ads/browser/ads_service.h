...
using OnGetBraveNewsAdCallback = base::OnceCallback<
...
class AdsService : public KeyedService {
 public:
  ...
  virtual void GetBraveNewsAd(const std::string& size,
                              OnGetBraveNewsAdCallback callback) = 0;

  virtual void OnBraveNewsAdEvent(
      const std::string& uuid,
      const std::string& creative_instance_id,
      const ads::mojom::BraveAdsBraveNewsAdEventType event_type) = 0;
