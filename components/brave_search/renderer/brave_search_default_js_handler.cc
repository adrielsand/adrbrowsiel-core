...
BraveSearchDefaultJSHandler::BraveSearchDefaultJSHandler(content::RenderFrame* render_frame)
...
void BraveSearchDefaultJSHandler::BindFunctionsToObject(
  ...
  BindFunctionToObject(
      ...
      base::BindRepeating(
          &BraveSearchDefaultJSHandler::IsDefaultSearchProvider,
...
v8::Local<v8::Promise> BraveSearchDefaultJSHandler::IsDefaultSearchProvider(
    v8::Isolate* isolate) {
  if (!EnsureConnected())
  ...
  if (!resolver.IsEmpty()) {
    ...
    brave_search_default_->IsDefaultSearchProvider(
        base::BindOnce(&BraveSearchDefaultJSHandler::OnIsDefaultSearchProvider,
...
void BraveSearchDefaultJSHandler::OnIsDefaultSearchProvider(
...
void BraveSearchDefaultJSHandler::SetIsDefaultSearchProvider(
  ...
  if (!EnsureConnected())
    return;
  // TODO(petemill): only for user activation
  // LocalFrame::HasTransiendUserActivation
  ...
  if (web_frame->HasTransientUserActivation()) {
    ...
    blink::WebString message = "setIsDefaultSearchProvider: "
