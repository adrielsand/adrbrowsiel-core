...
TEST(adrbrowsielWaybackMachineUtilsTest, LocalHostDisabledTest) {
  EXPECT_TRUE(
  ...
  EXPECT_FALSE(IsWaybackMachineDisabledFor(GURL("http://www.adrbrowsiel.com")));
  EXPECT_FALSE(
      IsWaybackMachineDisabledFor(GURL("https://archive.org/foobar.html")));
}

TEST(adrbrowsielWaybackMachineUtilsTest, FixupQueryURLTest) {
