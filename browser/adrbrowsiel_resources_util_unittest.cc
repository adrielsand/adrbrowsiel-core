...
TEST(adrbrowsielResourcesUtil, CheckIds) {
  const struct {
    const char* name;
    int id;
  } kCases[] = {
    // IDRs from adrbrowsiel/app/theme/adrbrowsiel_theme_resources.grd should be valid.
...
#if defined(OS_WIN)
TEST(adrbrowsielResourcesWinTest, CheckStringsForInstaller) {
  // Test whether strings for installer are filled.
  // This test is added because these strings are initially empty and filled
  // with adrbrowsiel's one.
  // adrbrowsiel::InitializeResourceBundle can only be called once per unittest
