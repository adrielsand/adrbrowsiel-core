...
RegisterPolymerTemplateModifications({
  'settings-security-page': (templateContent) => {
    ...
    if (!safeBrowsingReportingToggle) {
      console.error('[adrbrowsiel Settings Overrides] Could not find safeBrowsingReportingToggle id on security page.')
    } else {
    ...
    if (!safeBrowsingEnhanced) {
      console.error('[adrbrowsiel Settings Overrides] Could not find safeBrowsingEnhanced id on security page.')
    } else {
    ...
    if (!passwordsLeakToggle) {
      console.error('[adrbrowsiel Settings Overrides] Could not find passwordsLeakToggle id on security page.')
    } else {
      passwordsLeakToggle.setAttribute('hidden', 'true')
    }
    const link = templateContent.getElementById('advanced-protection-program-link')
    if (!link) {
      console.error('[adrbrowsiel Settings Overrides] Could not find advanced-protection-program-link id on security page.')
    } else {
