...
adrbrowsielSpellingOptionsSubMenuObserver::adrbrowsielSpellingOptionsSubMenuObserver(
...
void adrbrowsielSpellingOptionsSubMenuObserver::InitMenu(
    const content::ContextMenuParams& params) {
  // Let Chromium build the submenu.
  SpellingOptionsSubMenuObserver::InitMenu(params);

  // Assumptions:
  // 1. Use of spelling service is disabled in adrbrowsiel profile preferences.
...
void adrbrowsielSpellingOptionsSubMenuObserver::SetGtestMode(
    adrbrowsielSpellingOptionsSubMenuObserver::GtestMode mode) {
  gtest_mode_ = mode;
}
